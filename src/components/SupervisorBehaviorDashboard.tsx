...
export default function SupervisorBehaviorDashboard() {
  ...
  const handleUpdateStatus = async (record: BehaviorRecord, newStatus: string) => {
    ...
    if (error) {
      ...
      // إرسال إشعار آلي للمعلم الذي رفع الشكوى
      if (record.teacher_id) {
        await supabase.from('notifications').insert({
          sender_id: user?.id || 1,
          receiver_id: record.teacher_id,
          sender_role: 'supervisor',
          type: 'system',
          title: 'تحديث حالة شكوى',
          message: `تم تحديث حالة الشكوى التي رفعتها إلى: ${newStatus === 'investigating' ? 'جاري التحقيق' : 'تم الحل'}`,
  ...
  const studentTotals = useMemo(() => {
    ...
    behaviors.forEach(b => {
      ...
      totals[b.student_id].net += b.points; 
    });
    return totals;
  }, [behaviors]);
            ...
            return (
              <div key={item.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col justify-between overflow-hidden group">
                ...
                <div className="px-6 py-4 bg-slate-50/80 border-t border-slate-100 flex gap-2 items-center">
                  {item.status === 'active' && (
                    <button 
                      onClick={() => handleUpdateStatus(item, 'investigating')}
